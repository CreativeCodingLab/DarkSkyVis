...
function initHaloTree(url, firstTime) {
    ...
    var superHalos = [ ];  // add a list of SuperStructure Halos so we can keep track of them...
    ...
    oboe(url)
        .node("!.*", function(halo) {
            ...
            if (pid !== -1) {
                halo.isSub = true;
                superHalos.push(pid);
                console.log("\tadded", pid, "to", superHalos);
            }
            ...
            if (_materialsByPeriod.hasOwnProperty(time)){
                ...
                _materialsByPeriod[time] = new THREE.MeshPhongMaterial({
                    color: epochColorKey(time),
                    specular: epochColorKey(time),
                    shininess: 40,
            ...
            var mesh = createSphereGeometry(halo, sphereGeometry, sphereMaterial, true);
            ...
            if (!targetSet && (time >= EPOCH_HEAD && time <= EPOCH_TAIL) && sphereGroup.getObjectByName(id)) {
        ...
        .done(function() {
            ...
            propertyColorKey = d3.scale.linear()
                .domain([minMass/maxMass,
                    (maxMass/maxMass + minMass/maxMass) / 2,
                    maxMass/maxMass])
                .range([rgbToHex(255, 0, 0), rgbToHex(155, 100, 255), rgbToHex(255, 255, 0)]);

            console.log("propertyColorKey", propertyColorKey(minMass), propertyColorKey(maxMass));

            console.log("superHalos", superHalos);
            superHalos.forEach(function(id) {
                if (HaloLUT[+id])
                    HaloLUT[+id].isSuper = true
                else
...
function createSphereGeometry(halo, sphereGeometry, sphereMaterial, isTracked) {
    ...
    //    var color = epochColorKey(period)
    if (isTracked)
...
function createPathArrow(points, id, period) {
    ...
    direction.normalize();
    var arrow = new THREE.ArrowHelper(direction, src, length, epochColorKey(period))
    arrow.name = id;
    arrow.period = period;
    ...
    if (points && points.length > 1) {
        ...
        if (useSpline) {
            ...
            for (var i = 0; i <= numPoints; i++) {
                ...
                var color = epochColorKey(period + index);
...
function resetGlobalStructures(lock) {
    ...
    switch(lock){
        ...
        case 'trace':
            __traversed = {};
            resetGroup(traceGroup)
            break;

        case 'point':
            resetSprite(pointCloud)
            break;

        case "path":
            __traversed = {};
            resetGroup(linesGroup);
            break;
        case "inspect":
            __traversed = {};
            resetGroup(traceGroup);
            resetGroup(supGroup);
            resetSprite(mapGroup);
        default:
...
function prepareHalosForInspection() {
    alert("prepareHalosForInspection!!!");

    sphereGroup.children.forEach(function(mesh) {

        var halo = HaloLUT[+mesh.name];
        var scale = config.scale <= 0.0 ? 0.0001 : config.scale;
        var rs1 = halo.rvir * scale;

        var material = new THREE.SpriteMaterial({map: SubMap, color: rgbToHex(0,155,0), fog: true, transparent: true});
        var sprite = new THREE.Sprite(material);
        sprite.name = halo.id;
        sprite.period = +halo.time;
        sprite.rs1 = +halo.rvir;
        sprite.mvir = +halo.mvir;
        sprite.position.set(halo.x, halo.y, halo.z);
        sprite.visible = (+halo.time >= EPOCH_HEAD && +halo.time <= EPOCH_TAIL) ? config.showHalos : false;
        sprite.updateMatrix();

        if (halo.isSub && !halo.isSuper) {
            sprite.isSub = true
            sprite.isSuper = false;
            sprite.scale.set(0.1, 0.1, 0.1);
            mapGroup.add(sprite);
        }
        else if (halo.isSuper && !halo.isSub) {
            console.log("\tIts a super!");
            var geo = new THREE.SphereGeometry(1, 32, 32);
            var mat = new THREE.MeshPhongMaterial({
                    color: propertyColorKey(+halo.mvir/maxMass),
                    specular: epochColorKey(+halo.time),
                    shininess: 40,
                    shading: THREE.SmoothShading,
                    // blending: THREE.AdditiveBlending,
                    vertexColors: THREE.VertexColors,
            ...
            var mesh = createSphereGeometry(halo, geo, mat, false);
            mesh.isSub = false;
            mesh.isSuper = true;
            mesh.material.color.set(rgbToHex(75, 0, 130))
            mesh.scale.set(rs1, rs1, rs1);
            supGroup.add(mesh);
            ...
            sprite.isSub = false;
            sprite.isSuper = false;
            sprite.scale.set(rs1, rs1, rs1);
            sprite.material.color.set(rgbToHex(0,0,155))
            sprite.material.map = NormMap;
            mapGroup.add(sprite);
        }
